...
def get_dynamic_volume_threshold(market='KRW-XRP', period='day'):
...
def execute_buy_order():
    ...
    if response_buy and 'uuid' in response_buy:
        logger.info(f"{current_time} - 매수 주문: {buy_price}원, 수량: {volume} XRP")
    else:
...
def place_sell_order_on_buy(order_id, buy_price, volume):
    ...
    response_sell = place_order('KRW-XRP', 'ask', volume, sell_price, linked_order_id=order_id)
    ...
    if response_sell and 'uuid' in response_sell:
        ...
        logger.info(f"{current_time} - 매도 주문 실행: {sell_price}원, 수량: {volume} XRP (매수 주문 ID: {order_id})")
    else:
        logger.info(f"{current_time} - 매도 주문 실행 실패: {sell_price}원, 수량: {volume} XRP (매수 주문 ID: {order_id})")
...
if __name__ == "__main__":
    ...
    logger.info("Trading bot is running with time-based grid strategy for XRP. Press Ctrl+C to exit.")
