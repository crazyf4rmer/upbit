...
sell_order_count = {}  # {buy_order_id: count}
...
def process_completed_buy_order(buy_order_id, buy_price, volume):
    global pending_sell_count, sell_order_count
    
    completed_buy_orders[buy_order_id] = {'price': buy_price, 'volume': volume}
    pending_sell_count += 1
    sell_order_count[buy_order_id] = 1
    
    logger.info(f"매수 주문 처리 완료: ID {buy_order_id}, 가격: {buy_price:.2f}원, 수량: {volume} XRP")
    logger.info(f"현재 대기 중인 매도 주문 수: {pending_sell_count}")

def execute_and_manage_sell_orders():
    global active_sell_orders, completed_buy_orders, trade_in_progress, pending_sell_count, sell_order_count
    ...
    try:
                ...
                if cancel_result:
                    ...
                    if buy_order_id in completed_buy_orders:
                        ...
                        if is_profit_possible(buy_price, new_sell_price):
                            create_new_sell_order(buy_order_id, volume, new_sell_price)
                        else:
        ...
        # 새로운 매도 주문 생성
        while pending_sell_count > 0:
            for buy_order_id, buy_info in completed_buy_orders.items():
                if sell_order_count.get(buy_order_id, 0) > 0:
...
# 새로운 매도 주문 생성 함수
def create_new_sell_order(buy_order_id, volume, sell_price):
    global pending_sell_count, sell_order_count, active_sell_orders
    ...
    if response_sell and 'uuid' in response_sell:
        ...
        pending_sell_count -= 1
        sell_order_count[buy_order_id] -= 1
        logger.info(f"새로운 매도 주문 생성됨: 주문 ID {new_sell_order_id}, 가격: {sell_price:.2f}원, 수량: {volume} XRP")
    else:
        logger.debug(f"새로운 매도 주문 생성 실패: 가격 {sell_price:.2f}원, 수량: {volume} XRP")

# monitor_sell_orders 함수 수정
def monitor_sell_orders():
    global active_sell_orders, completed_buy_orders, total_invested, available_seed, pending_sell_count, sell_order_count
    while True:
        try:
            time.sleep(0.2)
                    ...
                    if status:
                        ...
                        if state == 'done':
                            ...
                            if buy_order_id in completed_buy_orders:
                                ...
                                if buy_order_id in sell_order_count:
                                    del sell_order_count[buy_order_id]
                            
                            del active_sell_orders[sell_id]
        except Exception as e:
            logger.error(f"매도 주문 모니터링 중 오류 발생: {e}", exc_info=True)

# 주기적으로 pending_sell_count와 실제 상황을 동기화하는 함수
def synchronize_pending_sell_count():
    global pending_sell_count, sell_order_count, completed_buy_orders, active_sell_orders
    
    while True:
        try:
            time.sleep(60)  # 1분마다 실행
            
            calculated_count = sum(sell_order_count.values())
            actual_count = len(completed_buy_orders) - len(active_sell_orders)
            
            if calculated_count != actual_count:
                logger.warning(f"대기 중인 매도 주문 수 불일치 감지: 계산된 수 {calculated_count}, 실제 수 {actual_count}")
                pending_sell_count = actual_count
                
                # sell_order_count 재계산
                sell_order_count = {buy_id: 1 for buy_id in completed_buy_orders}
                for sell_info in active_sell_orders.values():
                    buy_id = sell_info['buy_order_id']
                    if buy_id in sell_order_count:
                        sell_order_count[buy_id] = 0
                
                logger.info(f"대기 중인 매도 주문 수 동기화 완료: {pending_sell_count}")
        
        except Exception as e:
            logger.error(f"대기 중인 매도 주문 수 동기화 중 오류 발생: {e}", exc_info=True)
...
def execute_orderbook_based_trading():
    ...
    try:
        ...
        if available_seed >= buy_price * buy_volume:
            response_buy = place_order(market, 'bid', buy_volume, buy_price)
...
def main():
    ...
    threading.Thread(target=synchronize_pending_sell_count, daemon=True).start()
