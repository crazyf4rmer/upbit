...
def place_sell_order_on_buy(order_id, buy_price, volume, new_sell_price=None):
    ...
    target_profit_rate = 0.005  # 목표 수익률 (0.5%)
    ...
    if response_sell and 'uuid' in response_sell:
        ...
        if new_sell_price:
            ...
            logger.debug(f"재매도 주문 실행됨: {rounded_sell_price:.2f}원, 수량: {volume} UXLINK (매수 주문 ID: {order_id}, 재매도 주문 ID: {sell_order_id})")
        else:
            logger.debug(f"매도 주문 실행됨: {rounded_sell_price:.2f}원, 수량: {volume} UXLINK (매수 주문 ID: {order_id}, 매도 주문 ID: {sell_order_id})")
    else:
...
def execute_orderbook_based_trading():
    ...
    try:
        ...
        tick_size = get_tick_size(best_bid)  # 매수 가격에 맞는 틱 사이즈 가져오기

        # 매수 주문 가격 설정 (best_bid - 1 또는 2 틱)
        buy_prices = [best_bid - tick_size, best_bid - 2 * tick_size]
        # 매도 주문 가격 설정 (best_ask + 1 또는 2 틱)
        sell_prices = [best_ask + tick_size, best_ask + 2 * tick_size]
        ...
        for order in open_orders:
            if order.get('side') == 'bid':
                existing_buy_prices.add(float(order.get('price')))
            elif order.get('side') == 'ask':
                existing_sell_prices.add(float(order.get('price')))
        
        # 매수 주문 걸기
        for buy_price in buy_prices:
            if buy_price in existing_buy_prices:
                logger.debug(f"이미 매수 주문이 걸려있는 가격: {buy_price:.2f}원")
                continue  # 동일한 가격에 매수 주문이 이미 존재하면 스킵

            # 매수 시 사용할 금액 설정 (예: 10,000원)
            buy_amount = 10000.0
        ...
        for sell_price in sell_prices:
            if sell_price in existing_sell_prices:
                logger.debug(f"이미 매도 주문이 걸려있는 가격: {sell_price:.2f}원")
                continue  # 동일한 가격에 매도 주문이 이미 존재하면 스킵

            # 매도 시 물량 확인 (이미 보유한 비트코인 수량으로 설정 필요)
            # 간단히 예시로 10개 단위로 매도 주문을 걸어보겠습니다
            sell_volume = 10.0  # 예: 10 UXLINK
            # 실제 보유량을 확인하고 매도 물량을 설정하는 것이 좋습니다

            with open_order_lock():
                response_sell = place_order(market, 'ask', sell_volume, sell_price)
                if response_sell and 'uuid' in response_sell:
                    sell_order_id = response_sell['uuid']
                    logger.debug(f"매도 주문 생성됨: {sell_price:.2f}원, 수량: {sell_volume} UXLINK, 주문 ID: {sell_order_id}")
                else:
                ...
                for buy_id, sell_info in list(sell_orders.items()):
                    ...
                    if status:
                        ...
                        if state == 'done':
                            ...
                            current_market_price = get_current_market_price('KRW-UXLINK')
                            if current_market_price and current_market_price > trailing_price:
                                ...
                                with adjust_sell_lock:
                                    ...
                                    if response_new_sell and 'uuid' in response_new_sell:
                                        ...
                                        logger.warning(f"트레일링 매도 주문 실패: {rounded_new_sell_price:.2f}원, 수량: {volume} UXLINK, 매수 주문 ID: {buy_id}")
