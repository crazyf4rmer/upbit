...
def cancel_unfilled_sells_if_no_fills():
    ...
    while True:
        try:
            ...
            with open_order_lock():
                # 현재 미체결 매도 주문 목록 가져오기
                open_sell_orders = [info for info in sell_orders.values() if info['buy_price'] != 0.0 or info['buy_price'] == 0.0]  # 모두 포함

                if not open_sell_orders:
                    logger.debug("미체결 매도 주문이 없습니다.")
                    continue

                # 매도 주문을 생성 시간 기준으로 정렬 (가장 오래된 순)
                open_sell_orders_sorted = sorted(open_sell_orders, key=lambda x: x['created_at'])
                ...
                for sell_info in orders_to_cancel:
                    sell_id = sell_info['sell_order_id']
                    sell_price = sell_info['target_price']
                    logger.info(f"매도 주문 체결 지연으로 취소됨: 주문 ID {sell_id}, 가격: {sell_price:.2f}원")
                    cancel_response = cancel_order(sell_id)
                    if cancel_response and 'uuid' in cancel_response:
                        # 해당 매도 주문을 sell_orders에서 제거
                        for buy_id, info in list(sell_orders.items()):
                            if info['sell_order_id'] == sell_id:
                                del sell_orders[buy_id]
                                break
                    else:
...
# 실시간 주문 예약 매도 주문 추가
def schedule_profit_sell_order():
    """
    일반 거래를 유지하다가 특정 수익률에 도달 시 지정가 매도 주문을 생성하는 함수
    """
    market = 'KRW-SEI'
    while True:
        try:
            ...
            current_price = get_current_market_price(market)
            if current_price is None:
                logger.debug("현재 시장 가격을 가져올 수 없어 수익 매도 주문을 예약할 수 없습니다.")
                time.sleep(1)
                continue

            profit = (current_price - average_price) / average_price
            if profit >= 0.005:  # 0.5% 수익
                logger.info(f"수익률 0.5% 달성: 현재 수익률 {profit*100:.2f}%. 매도 주문을 예약합니다.")
                # 지연 시간(1~3초) 동안 수익률 유지 확인
                delay = np.random.randint(1, 4)
                logger.debug(f"수익률 확인을 위해 {delay}초 대기합니다.")
                time.sleep(delay)
                # 지연 후 다시 확인
                current_price_after_delay = get_current_market_price(market)
                if current_price_after_delay and ((current_price_after_delay - average_price) / average_price) >= 0.005:
                    logger.info(f"수익률이 지연 후에도 0.5% 이상입니다. 기존 매도 주문을 취소하고 지정가 매도 주문을 생성합니다.")
                    # 기존 매도 주문 모두 취소
                    cancel_all_sell_orders()
                    # 지정가 매도 주문 생성
                    with position_lock:
                        average_price = total_position['average_price']
                        total_volume = total_position['total_volume']
                    take_profit_price = average_price * 1.005  # 0.5% 수익
                    response_sell = place_order('KRW-SEI', 'ask', total_volume, take_profit_price, ord_type='limit')
                    if response_sell and 'uuid' in response_sell:
                        sell_order_id = response_sell['uuid']
                        sell_orders[f"profit_sell_{sell_order_id}"] = {
                            'sell_order_id': sell_order_id,
                        ...
                        logger.info(f"수익 실현을 위한 지정가 매도 주문 생성됨: 주문 ID {sell_order_id}, 가격: {take_profit_price:.2f}원, 수량: {total_volume} SEI")
                    else:
                        logger.warning("수익 실현 매도 주문 생성 실패.")
            time.sleep(1)
        except Exception as e:
            logger.debug(f"수익 매도 주문 예약 중 오류 발생: {e}")
            time.sleep(1)

# 도달 불가능한 매도 주문 취소 스레드 시작
def cancel_unfilled_sells_if_no_fills():
    """
    3분 간격으로 매도 주문이 하나도 체결되지 않았다면, 가장 오래된 미체결 매도 주문 2개(가능하면)를 취소하고 일반 매매를 재개합니다.
    """
    ...
    while True:
        try:
            ...
            with open_order_lock():
                # 현재 미체결 매도 주문 목록 가져오기
                open_sell_orders = [info for info in sell_orders.values()]
