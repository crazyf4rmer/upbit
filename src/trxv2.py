...
# 틱 사이즈 계산 함수 (고정된 틱 사이즈 0.1으로 설정)
def get_tick_size(price):
    """
    고정된 틱 사이즈를 반환합니다.
    TRX의 틱 사이즈는 0.1 KRW입니다.
    """
    return 0.1
...
def place_order(market, side, volume, price, linked_order_id=None, ord_type='limit'):
    ...
    try:
        ...
        if order_id:
            if side == 'bid':
                # 매수 주문 개별 로그 제거
                # logger.info(f"매수 주문 대기: {float(price):.2f}원, 수량: {volume} TRX, 주문 ID: {order_id}")
                pass  # 로그 제거
                ...
                logger.debug(f"매도 주문 생성됨: {float(price):.2f}원, 수량: {volume} TRX, 매수 주문 ID: {linked_order_id}, 매도 주문 ID: {order_id}")
            return data
        return data
...
def monitor_sell_orders():
    global sell_orders, trade_session_counter, trade_in_progress, trading_paused, total_buys, total_sells, cumulative_profit
    while True:
        try:
                    ...
                    if status:
                        ...
                        if state == 'done':
                            ...
                            if sell_price <= buy_price:
                                ...
                                # 매도 주문 재조정: 매수 가격보다 일정 마진을 더한 가격으로 재매도
                                new_sell_price = buy_price * 1.002  # 예: 0.2% 마진
                                tick_size = get_tick_size(new_sell_price)
                                rounded_new_sell_price = math.ceil(new_sell_price / tick_size) * tick_size

                                logger.debug(f"재매도 주문 설정: {rounded_new_sell_price:.2f}원 (틱 사이즈: {tick_size})")
                                
                                # 매도 주문 재조정을 위해 재조정 락 사용
                                with adjust_sell_lock:
                                    # 기존 sell_id는 이미 'done' 상태이므로 새로운 sell_order_id를 생성
                                    ...
                                    if response_new_sell and 'uuid' in response_new_sell:
                                        ...
                                        logger.info(f"재매도 주문 생성됨: {rounded_new_sell_price:.2f}원, 수량: {volume} TRX, 매수 주문 ID: {buy_id}, 재매도 주문 ID: {new_sell_order_id}")
                                    else:
                                        logger.warning(f"재매도 주문 실패: {rounded_new_sell_price:.2f}원, 수량: {volume} TRX, 매수 주문 ID: {buy_id}")
...
def place_sell_order_on_buy(order_id, buy_price, volume, new_sell_price=None):
    ...
    response_sell = place_order('KRW-TRX', 'ask', volume, rounded_sell_price, linked_order_id=order_id)

    if response_sell and 'uuid' in response_sell:
        sell_order_id = response_sell['uuid']
        if new_sell_price:
            # 재매도 주문일 경우 특별히 로그를 남김
            logger.debug(f"재매도 주문 실행됨: {rounded_sell_price:.2f}원, 수량: {volume} TRX (매수 주문 ID: {order_id}, 재매도 주문 ID: {sell_order_id})")
        else:
            logger.debug(f"매도 주문 실행됨: {rounded_sell_price:.2f}원, 수량: {volume} TRX (매수 주문 ID: {order_id}, 매도 주문 ID: {sell_order_id})")
    else:
        logger.debug(f"매도 주문 실행 실패: {rounded_sell_price:.2f}원, 수량: {volume} TRX (매수 주문 ID: {order_id})")
        with trade_lock:
            trade_in_progress = False
...
def execute_buy_order_real_time():
    ...
    try:
        ...
        volume = '30'  # TRX 매수 수량 (필요에 따라 조정 가능)
        ...
        with open_order_lock():
            ...
            if response_buy and 'uuid' in response_buy:
                ...
                logger.debug(f"매수 주문 체결 대기: 매수 가격 {buy_price:.2f}원, 수량: {volume} TRX, 주문 ID: {buy_order_id}")
                    ...
                    if elapsed > timedelta(minutes=10):
                        ...
                        if order_id:
                            ...
                            if cancel_response and 'uuid' in cancel_response:
                                logger.info(f"10분 이상 미체결 되어 취소된 매수 주문: 주문 ID {order_id}, 매수 가격: {order.get('price')}원, 수량: {order.get('volume')} TRX")
                            else:
