...
def partial_market_sell_strategy(trading_manager, buy_order_id):
    ...
    # 시장가 매도 실행
    response_market_sell = place_order('KRW-XRP', 'ask', market_sell_volume, 0, ord_type='market')
    
    if response_market_sell and 'uuid' in response_market_sell:
        logger.info(f"부분 시장가 매도 주문 실행: ID {response_market_sell['uuid']}, 수량: {market_sell_volume} XRP")
        ...
        if sell_status and sell_status.get('state') == 'done':
            ...
            if remaining_volume > 0:
                ...
                if orderbook:
                    ...
                    if new_sell_price:
                        response_limit_sell = place_order('KRW-XRP', 'ask', remaining_volume, new_sell_price)
                        if response_limit_sell and 'uuid' in response_limit_sell:
                            ...
                            logger.info(f"새 지정가 매도 주문 생성: ID {new_sell_order['uuid']}, 가격: {new_sell_price:.2f}원, 수량: {remaining_volume} XRP")
                        else:
