...
# 수익률 설정 (0.2%)
PROFIT_TARGET_RATE = 0.002  # 0.2% 상승 시
...
# 전역 변수 및 데이터 구조
sell_orders = {}  # {buy_order_id: {'sell_order_id': sell_id, 'buy_price': buy_price, 'volume': volume, 'target_price': target_price}}
...
def place_order(market, side, volume, price, linked_order_id=None, ord_type='limit'):
    global sell_orders  # Moved to top of function
    params = {
        'market': market,
        'side': side,
    ...
    try:
        ...
        if order_id:
            if side == 'bid':
                ...
                logger.debug(f"매도 주문 생성됨: {float(price):.2f}원, 수량: {volume} SEI, 매수 주문 ID: {linked_order_id}, 매도 주문 ID: {order_id}, 목표 가격: {float(price) * (1 + PROFIT_TARGET_RATE):.2f}원")
            return data
        return data
    except requests.exceptions.Timeout:
...
def place_sell_order_on_buy(order_id, buy_price, volume, new_sell_price=None):
    global trade_in_progress  # Moved to top of function
...
def place_grid_buy_orders(base_price, tick_size, levels):
    global total_invested  # Moved to top of function
...
def place_grid_sell_orders(base_price, tick_size, levels):
    ...
    for price in sell_prices:
        ...
        sell_volume = current_sei_balance / (levels * 2)  # 예시: 보유 SEI를 레벨 수의 두 배로 나눠 각 주문에 배정
        sell_volume = round(sell_volume, 6)

        if sell_volume < 0.001:  # 최소 매도 물량 설정
            logger.debug(f"매도 물량이 너무 작아 매도 주문을 걸지 않습니다: {sell_volume} SEI")
            continue

        with open_order_lock():
            response_sell = place_order('KRW-SEI', 'ask', sell_volume, price)
            if response_sell and 'uuid' in response_sell:
                sell_order_id = response_sell['uuid']
...
def execute_orderbook_based_trading():
    global last_order_time, trade_in_progress, trading_paused, total_invested  # Moved to top of function
    market = 'KRW-SEI'  # SEI 시장 심볼로 변경
    ...
    try:
        ...
        place_grid_buy_orders(base_price, tick_size, GRID_LEVELS)  # 그리드 매수 주문 배치
        place_grid_sell_orders(base_price, tick_size, GRID_LEVELS)  # 그리드 매도 주문 배치
...
def monitor_sell_orders():
    global sell_orders, trade_session_counter, trade_in_progress, trading_paused, total_buys, total_sells, cumulative_profit, total_invested  # Moved to top of function
    while True:
        try:
            ...
            with open_order_lock():
                for buy_id, sell_info in list(sell_orders.items()):
                    ...
                    target_price = sell_info.get('target_price', buy_price * (1 + PROFIT_TARGET_RATE))
...
def manage_open_orders():
    global trading_paused  # Moved to top of function
    while True:
        try:
...
if __name__ == "__main__":
    start_threads()
    logger.info("트레이딩 봇이 SEI를 대상으로 0.2% 수익률 달성 시 전부 매도하도록 실행 중입니다. 종료하려면 Ctrl+C를 누르세요.")
